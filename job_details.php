...
function jp_salary_label($job) {
    ...
    if ($salaryMin && $salaryMax) {
        return trim($currency . ' ' . $salaryMin . ' - ' . $salaryMax);
    }

    if ($salaryMin) {
        return trim($currency . ' ' . $salaryMin . '+');
    }
...
if ($job) {
    ...
    try {
        ...
        // Media is optional; the details page can render without it.
    }
    ...
    try {
        ...
        // View tracking should never block the public details page.
    }
...
function showHtmlAlert(message, type) {
    ...
    alertBox.removeClass('alert-success alert-danger alert-warning alert-info d-none').addClass('alert-' + safeType);
    $('#dynamic-alert-msg').text(message);
    window.clearTimeout(window.jobDetailAlertTimer);
    window.jobDetailAlertTimer = window.setTimeout(function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.status === 'success') {
                    ...
                    showHtmlAlert(response.message || 'Unable to save the rating.', 'danger');
                }
            },
            error: function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.status === 'success') {
                    ...
                    showHtmlAlert(response.message || 'Unable to post the comment.', 'danger');
                }
            },
            error: function() {
...
<div id="job-detail-data" data-job-id="<?php echo (int)$job_id; ?>" data-logged-in="<?php echo $is_logged_in ? '1' : '0'; ?>"></div>
    ...
    <div class="row g-4">
        <div class="col-lg-8">
            <div class="detail-media mb-4">
                <?php if ($media['image']): ?>
                    <img src="<?php echo htmlspecialchars($media['image']); ?>" alt="<?php echo htmlspecialchars($media['image_alt']); ?>" onerror="this.classList.add('d-none'); this.nextElementSibling.classList.remove('d-none');">
                <?php endif; ?>
            ...
            <div class="detail-card p-4 mb-4">
                <div class="d-flex flex-wrap align-items-start justify-content-between gap-3 mb-3">
                    <div>
                        <span class="badge text-bg-light border mb-3 px-3 py-2"><?php echo htmlspecialchars($job['employment_type']); ?></span>
                    ...
                    <div class="job-card-logo lg">
                        <?php if ($logo_path): ?>
                            <img src="<?php echo htmlspecialchars($logo_path); ?>" alt="<?php echo htmlspecialchars($job['company_name']); ?> logo" onerror="this.classList.add('d-none'); this.nextElementSibling.classList.remove('d-none');">
                ...
                <div class="detail-meta">
                    ...
                    <div class="detail-meta-item">
                        ...
                        <div class="fw-bold"><i class="bi bi-grid me-1"></i><?php echo htmlspecialchars($job['category_name']); ?></div>
                    </div>
                    <div class="detail-meta-item">
                        <div class="small text-muted">Posted</div>
                        <div class="fw-bold"><i class="bi bi-calendar3 me-1"></i><?php echo htmlspecialchars($posted_label); ?></div>
                    </div>
                </div>
            </div>
            ...
            <div class="detail-card p-4 mb-4">
                ...
                <?php if (!empty($job['requirements'])): ?>
                    <h2 class="h5 fw-bold border-bottom pb-2 mt-4 mb-3">Candidate Requirements</h2>
                    <p class="text-secondary lh-lg small"><?php echo nl2br(htmlspecialchars($job['requirements'])); ?></p>
                <?php endif; ?>
            ...
            <div class="detail-card mb-4 overflow-hidden">
                ...
                <div class="card-body p-4 bg-white">
                    <div class="rating-section mb-4 text-center p-3 soft-panel">
                        ...
                        <div class="star-rating my-2 fs-3 text-warning" aria-label="Rate this job out of five stars">
                            <?php for ($i = 1; $i <= 5; $i++): ?>
                                <span class="star-item mx-1" role="button" tabindex="0" data-value="<?php echo $i; ?>"><?php echo $i <= $rounded_avg ? "&#9733;" : "&#9734;"; ?></span>
                            <?php endfor; ?>
                        </div>
                        <p class="text-muted small mb-0">
        ...
        <div class="col-lg-4">
            <aside class="detail-card p-4 sticky-top job-summary-card">
                ...
                <ul class="list-unstyled mb-4 small text-secondary">
                    ...
                    <li class="mb-2"><i class="bi bi-clock me-2 text-primary"></i><strong>Job Type:</strong> <?php echo htmlspecialchars($job['employment_type']); ?></li>
                ...
                <?php if (!empty($job['application_url'])): ?>
                    <a class="btn btn-success w-100 py-2 mb-3" href="<?php echo htmlspecialchars($job['application_url']); ?>" target="_blank" rel="noopener">
